...
stream_to_file_progress() {
  # Read stdin, write to OUT_PATH atomically with progress.
  # Usage: stream_to_file_progress <expected_size_or_0> <out_path> <label>
  expected="${1:-0}"
  out_path="$2"
  label="$3"
  ...
  chunk=65536
  while :; do
    # Read one chunk from stdin, append to tmp_out, count actual bytes read.
    nbytes="$(dd bs="$chunk" count=1 2>/dev/null | tee -a "$tmp_out" | wc -c | tr -d ' ')"
    if [ "${nbytes:-0}" -eq 0 ] 2>/dev/null; then
      break
    fi
    done_bytes=$((done_bytes + nbytes))
    if [ "${expected:-0}" -gt 0 ] 2>/dev/null; then
      pct=$((done_bytes * 100 / expected))
      if [ "$pct" -gt 100 ]; then pct=100; fi
      print_progress_line "${label} ${pct}% (${done_bytes}/${expected} bytes) $out_path"
    else
      print_progress_line "${label} ${done_bytes} bytes $out_path"
    fi
  done
  finish_progress_line
...
print_help() {
  cat <<'EOF'
Usage:
  transfer.sh --type <upload|download> [options]

Common options:
  --type <upload|download>   Required
  --server <url>             Default: http://127.0.0.1:8000 (or env SERVER)
  --game <name>              Default: yuanshen (or env GAME)
  --account <name>           Default: gamer_name (or env ACCOUNT)
  --chunk-size <bytes>       Default: 4194304 (or env CHUNK_SIZE)
  -h, --help             Show this help

Upload options:
  --file <absolute_local_path>    Required when --type upload
    Server target path will be: <game>/<account>/<absolute_path>

Download options:
  --mkdir                         Auto-create missing local directories (default OFF)
  --from <str> --to <str>         Replace within directory path only (not filename)

Examples:
  # Upload
  transfer.sh --type upload --server http://192.168.1.10:8000 --game yuanshen --account gamer_name \
  ...
  find /Users/sunmeng/file_trans/ -type f -name '*.md' -print0 \
    | xargs -0 -n 1 sh -c 'sh transfer.sh --type upload --server http://192.168.1.10:8000 --game star_rail --account gamer_name1 --file $1

  # Download (strict: do not create dirs)
  transfer.sh --type download --server http://192.168.1.10:8000 --game star_rail --account gamer1

  # Download and auto-create missing dirs
  transfer.sh --type download --server http://192.168.1.10:8000 --game star_rail --account gamer1 --mkdir
...
download_dir_overwrite() {
  ...
  while IFS= read -r name; do
    ...
    if [ -n "$expected_size" ] && [ "$expected_size" -gt 0 ] 2>/dev/null && has_cmd pv; then
      ...
      # No pv available: stream in chunks and compute percent ourselves.
      label="Downloading: done=${done_items}/${total_items} remaining=${remaining} file"
      if ! tar_extract_stdout "$tar_path" "$member_name" | stream_to_file_progress "${expected_size:-0}" "$out_path" "$label"; then
        status=1
        echo "$normalized" > "$pending_path"
        while IFS= read -r rest; do
          case "$rest" in
...
if [ -z "$TYPE" ]; then
  echo "ERROR: --type is required (upload|download)" >&2
  echo "HINT: use --help" >&2
  exit 2
fi
...
case "$TYPE" in
  upload)
    if [ -z "$FILE" ]; then
      echo "ERROR: --file is required for upload" >&2
      echo "HINT: use --help" >&2
      exit 2
    fi
    upload_file "$FILE"
    ;;
  download)
    if [ -z "${GAME:-}" ] || [ -z "${ACCOUNT:-}" ]; then
      echo "ERROR: download requires --game and --account (or env GAME/ACCOUNT)" >&2
      echo "HINT: use --help" >&2
      exit 2
    fi
    if [ -n "$FROM_STR" ] || [ -n "$TO_STR" ]; then
    ...
    if [ -n "$FROM_STR" ]; then
      args="$args --from $FROM_STR --to $TO_STR"
    fi
    # shellcheck disable=SC2086
    download_dir_overwrite "${GAME}/${ACCOUNT}" $args
    ;;
  *)
    echo "ERROR: invalid --type: $TYPE (expected upload|download)" >&2
    exit 2
    ;;
esac
